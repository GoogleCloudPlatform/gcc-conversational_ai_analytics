view: dfcx_test_runs_results {
  label: "DFCX Test Runs"

  derived_table: {
    explore_source: dfcx_test_cases_results {
      column: test_run_time { field: dfcx_test_cases_results.test_run_time}
      column: test_run_guid {}
      column: agent_id {}
      column: agent_display_name {}
      column: test_cases { field: dfcx_test_cases_results.total_test_cases }
      column: test_cases_passed { field: dfcx_test_cases_results.total_test_cases_passed }
    }
  }

  dimension: primary_key {
    hidden: yes
    primary_key: yes
    type: string
    sql: ${test_run_guid} ;;
  }

  dimension: agent_display_name {
    type: string
    sql: ${TABLE}.agent_display_name ;;
  }

  dimension: agent_id {
    type: string
    sql: ${TABLE}.agent_id ;;
  }

  dimension: test_run_guid {
    type: string
    sql: ${TABLE}.test_run_guid ;;
  }

  dimension_group: test_run {
    type: time
    timeframes: [raw, time, date, week, month, quarter, year]
    sql: ${TABLE}.test_run_time ;;
  }

  dimension: test_cases {
    type: number
    sql: ${TABLE}.test_cases ;;
  }

  dimension: test_cases_passed {
    type: number
    sql: ${TABLE}.test_cases_passed ;;
  }

  dimension: test_run_filter {
    type: string
    sql: CONCAT(${test_run_raw},${test_run_guid}) ;;
    order_by_field: test_run_raw
  }

  dimension: test_pass_rate {
    type: number
    sql: SAFE_DIVIDE(${test_cases_passed},${test_cases}) ;;
    value_format_name: percent_1
  }

  measure: total_tests_runs {
    type: count
  }

  measure: average_test_pass_rate {
    type: average
    sql: ${test_pass_rate} ;;
  }

  set: dfcx_test_run {
    fields: [test_run_time, test_run_guid, test_pass_rate, dfcx_test_cases_results.total_test_cases, dfcx_test_cases_results.total_test_cases_passed, dfcx_test_cases_results.total_test_cases_failed]
  }

  drill_fields: [dfcx_test_run*]

}
